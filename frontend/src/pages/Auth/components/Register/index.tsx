...
import { useMultistepForm } from 'shared/libs/useMultistepForm';
        ...
        <>
            ...
            <form onSubmit={onSubmit} className={styles.form}>
                ...
                <div className={styles.buttonDiv}>
                    {!isFirstStep && <Button label='Назад' buttonStyle='primary' onClick={back} />}

                    <Button label={isLastStep ? 'Готово' : 'Далее'} buttonStyle='primary' type='submit' />
                </div>
            </form>
            {isFirstStep && <Help className='mt-10' message='Есть аккаунт?' linkMessage='Тыкни на меня!' link='/auth/login' />}
        </>
    )
}
